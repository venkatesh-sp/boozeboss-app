        ...
        {code ? (
            ...
            <StyledRow justify="space-between">
              ...
              <StyledColumn align="flex-end">
                <StyledRow justify="flex-end">
                  ...
                  <Icon
                    icon="circle"
                    style={{ color: '#c2b90a', margin: '0 0.5em 0 0.5em' }}
                  />
                  <b>
                    (~ {user.location.currency}{' '}
                    {Math.round(credits / user.location.currency_conversion)})
                  </b>
                </StyledRow>
              </StyledColumn>
            </StyledRow>
          ...
          <CreditsContainer>
                ...
                <StyledColumn align="flex-start">
                  {!isLoading && (
                    <IconButton
                      ...
                      onClick={() => this.handleChangeCredits('remove')}
                    />
                  )}
                </StyledColumn>
                ...
                <StyledColumn align="flex-end">
                  {!isLoading && (
                    <IconButton
                      ...
                      color="green"
                      icon={<Icon icon="plus" />}
                      onClick={() => this.handleChangeCredits('add')}
                    />
                  )}
                </StyledColumn>
              </StyledRow>
            ...
            {isLoading && (
              <LoaderSection>
                <Loader
                  content="Processing payment. Plase wait a few seconds."
                  vertical
                />
              </LoaderSection>
            )}
            <PaymentMethods isLoading={isLoading}>
              <CreditsPayment>
                <Button
                  color="green"
                  block
                  onClick={this.handlAddCreditsWithCode}
                >
                  <b>Pay with cash</b>
                  <Icon icon="qrcode" style={{ marginLeft: '10px' }} />
                </Button>
              </CreditsPayment>
              <CreditsPayment>
                {user && (
...
function mapDispatchToProps(dispatch) {
  return {
    addCreditsWithPaypal: (purchase, history) =>
      dispatch(addCreditsWithPaypal(purchase, history)),
    addCreditsWithQR: (purchase, history) =>
      dispatch(addCreditsWithQR(purchase, history)),
