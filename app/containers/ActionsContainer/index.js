...
import { makeSelectUser, makeSelectScope } from '../App/selectors';
...
export class ActionsContainer extends React.Component {
  ...
  render() {
    const { actions, scope } = this.props;
    return (
      <div>
        <Helmet>
        ...
        <StyledDiv>
          ...
          {actions && actions.length > 0 ? (
            <ActionsSection>
              {actions.map(action => {
                if (
                  action.action === 'RECEIVED_TRANSFER' ||
                  action.action === 'SENT_TRANSFER'
                ) {
            ...
            <Message
              type="info"
              description="All your purchases, orders and transfers will appear here."
            />
          )}
        </StyledDiv>
